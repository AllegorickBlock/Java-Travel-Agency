...
public class SetupCnntroller {
    ...
    private void generateListStep() {
        ArrayList<Step> steps = stepList.getStepList();
        for (Step step:steps) {
            if(step instanceof Plane) {
                generatePlane(step);
            }
            else generateHotel(step);
        }
    }

    private void generateHotel(Step step) {

        Hotel hotel = (Hotel) step;
    ...
    private void generatePlane(Step step) {
