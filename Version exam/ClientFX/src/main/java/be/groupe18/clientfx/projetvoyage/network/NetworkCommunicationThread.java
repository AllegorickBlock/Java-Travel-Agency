...
public class NetworkCommunicationThread extends Thread{
    ...
    public void run() {
        ...
        while(isRunning){
            try {
                AbstractFunction function = objectSocket.read();
                function.accept(new Function() {
                    @Override
                    public void lockTrip(LockingTrip trip) {
                        listener.lockTrip(trip);
                    }

                    @Override
                    public void numberClient(NumberClient numberClient) {
                        listener.updateNumberClient(numberClient);
                    }
                    ...
                    @Override
                    public void removeTrip(RemoveTrip trip) {listener.removeTrip(trip);}
                });
    ...
    * */
    ...
    public void sendWorkingTrip(int indexOf) throws IOException {
        objectSocket.write(new LockingTrip(indexOf));
    }
    ...
    public void sendNewTrip(StepList stepList){
        try {
            objectSocket.write(new NewTrip());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    */
    public void updateTrip(StepList stepList, int index) {
        try {
            objectSocket.write(new UpdateTrip(index,stepList));
    ...
    public void removeTrip(StepList stepList, int index) {
        try {
            objectSocket.write(new RemoveTrip(index, stepList));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public interface Listener {
        ...
        void lockTrip(LockingTrip read);
        void newTrip();
        ...
        void updateNumberClient(NumberClient numberClient);
    }
}
